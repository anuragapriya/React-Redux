   ...
   // const baseUrl = 'https:/localhost:7017/api/Marketer';
        ...
        return createAsyncThunk(
            ...
            async (transformedData, { rejectWithValue }) => {
                try {
                    ...
                    const response = await trackPromise(fetchWrapper.get(url.toString(),transformedData));
                    return response;
                } catch (error) {
    ...
    function update() { 
        return createAsyncThunk( 
            ...
            async (transformedData , { rejectWithValue }) => {
                try {
                    const response = await trackPromise(fetchWrapper.post(`${baseUrl}/UpdateGroupNominations`, transformedData));
                    return response;
                } catch (error) {
                    return rejectWithValue(error);
                }
